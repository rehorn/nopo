...
function cloneProject(callback){
    ...
    logger.log("clone project to [ " + setting.sourceRoot.grey + ' ]');
    logger.log("only allow filetype: [ " + setting.allowExt.join(',').grey + ' ]');
    var source = ["**"];
    if(setting.applyFileFilter){
        source = util.formatSourceFileType(source, setting.allowExt.join(','));
    }
...
function cleanBuildTemp(){
    ...
    if(setting.cloneProject){
        logger.log("clean clone project folder [ " + setting.sourceRoot.grey + ' ]');
        file.delete(setting.sourceRoot);
    }
};
