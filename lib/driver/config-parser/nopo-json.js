...
var parseTask = function(taskConfig, taskKey){
    ...
    if(taskConfig){
        ...
        oSource = _.map(oSource, function(item){
            item = item || getConfigValue('defaultSource');
            // 去除source root，作用不大
            // 重新添加上，防止翻译时修改源文件，会将文件复制到临时目录，再进行编译
            if(!file.doesPathContain(getConfigValue('targetRoot'), item) && (getConfigValue('targetRoot') !== item)){
                var exclusion = item.indexOf('!') === 0;
                if(exclusion){
                    item = item.slice(1);
                }
                item = path.join(getConfigValue('cloneRoot'), item);
                if(exclusion){
                    item = '!' + item;
                }
            }
