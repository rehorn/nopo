...
var parseTask = function(taskConfig, taskKey){
    ...
    if(taskConfig){
        ...
        oSource = _.map(oSource, function(item){
            // return path.join(getConfigValue('SOURCE_ROOT'), item || config.DEAFAULT_SOURCE);
            // 去除source root，作用不大
            return item || config.DEAFAULT_SOURCE;
        });
        ...
        oTarget = path.join(getConfigValue('TARGET_ROOT'), oTarget || config.DEAFAULT_TARGET),
