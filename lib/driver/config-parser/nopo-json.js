...
var parseTask = function(taskConfig, taskKey){
    ...
    if(taskConfig){
        var cmd = taskConfig.cmd || config.DEAFAULT_CMD,
            oSource = path.join(config.SOURCE_ROOT, taskConfig.source || config.DEAFAULT_SOURCE),
            oTarget = path.join(config.TARGET_ROOT, taskConfig.target || config.DEAFAULT_TARGET),
