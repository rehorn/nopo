...
var normalize = exports.normalize= function(filepath) {
...
exports.getFilesSourceTargetMap = function(source, target, isFlatten){
    var config = require('./config');
    isFlatten = _.isUndefined(isFlatten) ? false : isFlatten;
    var srcFiles = exports.expandFiles(source) || [];
    var stMap = {
        files: srcFiles
    };
    ...
    srcFiles.forEach(function(fileInput){
        ...
        if(isFlatten){
            ...
            var dirname = path.dirname(fileInput);
            var fileName = path.basename(fileInput);
            var relFilePath = path.relative(config.setting.sourceRoot, dirname);
            fileOutput = path.join(target, relFilePath, fileName);
        }
