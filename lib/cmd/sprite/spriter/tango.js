...
function saveSprite(){
    ...
    var seriesTaskQueue = _.map(folderLevelOne, function(item){
        ...
        var seriesTaskFunc = function(callback){
            ...
            var imagesList = file.expandFiles(util.formatSourceFileType(item, imageType, recursive));
...
function getSpiltBackgroundProperty(splitBackground, property, idx){
    ...
    return splitBackground[property] ? (splitBackground[property].split(',')[idx] ? splitBackground[property]) : defaultValue;
};
...
function updateRulesBackground(rules, from, to){
    ...
    var visit = function(rules, backgroundsRules, extra) {
        rules.forEach(function(rule) {
            ...
            splitBackground['background'] && mergeSplitBackground(splitBackground, background.analyse(splitBackground['background']));
            delete splitBackground['background'];
                ...
                if(bgImgs){
                    ...
                    _.each(bgImgs.split(','), function(bgImg, idx){
                        ...
                        if(ignoreNetworkRegexp.test(imageUrl) || 
                            ...
                            bgPositions.push((pos[0] ? pos[0] + 'px' : 0) + ' ' + (pos[1] ? pos[1] + 'px' : 0));
                            newBgImgs.push(bgImg);
                        }
                    });
