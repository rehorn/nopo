...
var _source, _target, _callback, spriteList, cssFiles, cssFilesMap;
var _options = {},
...
function saveSprite(cb){
    ...
    var st = file.normalizeTaskSourceTarget(_options.imgRoot, _options.output);
    _options.imgRoot = st.source;
    _options.output = st.target;
    var imgRootRaw = st.sourceRaw;

    // 使用
    var src = util.normalize(_options.imgRoot[0]).replace('/**', '/');
    var ioMap = file.getFileIOMaps(src, _options.output, { inclueDir: 1, includeEmpty:1, basePath:imgRootRaw });
    // console.log(ioMap)
    var dest = ioMap.firstDirOutput;
    ...
    var seriesTaskQueue = _.map(folderLevelOne, function(item){
        ...
        var seriesTaskFunc = function(callback){
            // console.log(item, _options.imgRoot, imgRootRaw)
            item = item + '/';
            ...
            var spriteUrl = path.join(dest, _options.prefix + name + '.png');
...
function updateRulesBackground(rules, from, to){
    ...
    var cssFrom = path.dirname(from),
        cssTo = path.dirname(to);
            ...
            rule.declarations.filter(function(declaration){
                ...
                if(bgImgs){
                    ...
                    _.each(bgImgs.split(','), function(bgImg, idx){
                        ...
                        absImageUrl = path.resolve(cssFrom, imageUrl);
                        ...
                        // 跳过特殊网址: 远程网址 || 图片不存在 || 图片不在imgRoot下 || 不以 url 开始的background-image
                        // 只进行复制：使用center/right/left定位 || 使用repeat || 使用 repeat-x|-y，并且切图不在-x,-y文件夹下
                        // 只进行复制：图片不在sprite切图列表中 
                        if(!imageRegexp.test(bgImg) ||
                            ignoreNetworkRegexp.test(imageUrl) || 
                            ...
                            var relpath = path.relative(process.cwd(), absImageUrl);
                            var ioMap = file.getFileIOMaps(relpath, _target);
                            var target = ioMap.firstFileOutput;
                            file.copy(absImageUrl, target);

                            newImageUrl = path.relative(cssTo, target);
                            ...
                            isReplaced = true;
                            isPosUpdated = true;
                            newImageUrl = path.relative(cssTo, coordinate.sprite);
                            newImageUrl = newImageUrl.replace(/\\/g, '/');
                            bgImg = bgImg.replace(imageUrl, newImageUrl);
                            declaration.value = declaration.value.replace(imageUrl, newImageUrl);
                            pos = pos.split(/\s+/);
...
function replaceCss(cb){
    ...
    cssFiles.forEach(function(cssFile){
        ...
        // var cssFile = '../nopo-test3/css/test.css';
        ...
        try{
            ...
            logger.warn('css parser error: [ ' + toFile.grey + ' ]');
            file.copy(cssFile, toFile);
            logger.log('copy css to [ ' + toFile.grey + ' ]');
        }
