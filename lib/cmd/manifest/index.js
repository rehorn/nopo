...
var pickupCss = function(html, manifest, config){
    var url = html.from;
    var content = fs.readFileSync(url).toString();
...
exports.run = function(cmdConfig, callback) {
    ...
    options.cache = {};
    var ioMap = file.getFileIOMaps(util.formatFileType(source, 'html'), target);
    _.each(ioMap.files, function(from){
        var basename = path.basename(from, '.html');
        options.cache[basename] = [{
            from: from,
