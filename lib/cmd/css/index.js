...
function parseImageUrl(url, cssFrom, cssTo) {
    ...
    console.log(cssFrom, imagePath)
    var relPath = path.relative(cssFrom, absolutePath);
    var toAbsolutePath = path.resolve(path.join(cssTo, imagePath));
    var toRelPath = path.relative(cssTo, toAbsolutePath);
...
function enhanceCss(css, cssFrom, cssTo) {
    ...
    (css.match(urlPattern) || []).forEach(function(match) {
        ...
        if(pathInfo.exists && _opt.addMark){
    ...
    data.result = css.replace(urlPattern, function(match, url){
        ...
        var st = file.normalizeTaskSourceTarget(_opt.exclude, '');
        if(_opt.dataUri && 
            !file.isMatch(st.source, path.relative(process.cwd(), pathInfo.absolute)) &&
        ...
        return ['url(', host , pathInfo.stamp || pathInfo.toRelative, ')'].join('');
    });
    ...
    if(_opt.addMark){
        ...
        data.result += '\r\n#nopo-auto-generated-mark{content: "' + encrypted.digest('hex') + '";}';
    }
...
function addFileStamp(pathInfo){
    ...
    if(_opt.stamp == 'md5'){
        ...
        pathInfo.stamp = toStampName(pathInfo.toRelative, stamp);
    }else if(_opt.stamp == 'mtime'){
        stamp = String(Date.parse(fs.statSync(pathInfo.absolute).mtime) / 1000).substring(0, 8);
        pathInfo.stamp = toStampName(pathInfo.toRelative, stamp);
    }
    if(_opt.rev){
        addRevFile(pathInfo.toRelative, pathInfo.toAbsolute, stamp);
    }
    return pathInfo;
};
...
exports.run = function(cmdConfig, callback) {
    ...
    var fileMap = file.getFileIOMaps(source, target);

    fileMap.files.forEach(function(item) {
        var from = item,
            to = fileMap.filesMap[from];
